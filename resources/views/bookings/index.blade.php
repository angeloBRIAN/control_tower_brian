...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h1><i class="bi bi-calendar-check me-2"></i>Bookings</h1>
        <p class="text-muted">Total: {{ $bookings->total() }} bookings</p>
    </div>
...
<div class="card mb-4">
    <div class="card-body">
        <form method="GET" class="row g-2 align-items-center" id="searchForm">
            <div class="col-md-3">
                <input type="text" name="search" class="form-control form-control-sm" placeholder="Search Plate, Customer..." value="{{ request('search') }}">
            </div>
            <div class="col-md-2">
        ...
        <div class="table-responsive">
            <table class="table table-hover table-bordered table-sm mb-0" id="dataTable">
                <thead class="table-dark">
                    @php
                        $currentSort = request('sort', 'booking_date');
                ...
                <tbody id="tableBody">
                    ...
                    <tr>
                        <td data-col="no">{{ $bookings->firstItem() + $index }}</td>
                        <td data-col="wip"><span class="fw-bold text-success">{{ $booking->wip }}</span></td>
                        <td data-col="plate"><span class="fw-bold text-primary">{{ $booking->plate_number }}</span></td>
                        <td data-col="customer">{{ $booking->customer_name }}</td>
                        <td data-col="phone">{{ $booking->customer_phone }}</td>
                        <td data-col="date">{{ $booking->booking_date?->format('d/m/Y') }}</td>
                        <td data-col="time">{{ $booking->booking_time }}</td>
                        <td data-col="service">{{ $booking->service_type }}</td>
                        <td data-col="foreman">{{ $booking->foreman }}</td>
                        <td data-col="sa">{{ $booking->service_advisor }}</td>
                        <td data-col="notes" class="text-truncate" style="max-width: 150px;">{{ $booking->notes }}</td>
                        <td data-col="status">
                            <span class="badge bg-{{ $booking->status === 'completed' ? 'success' : ($booking->status === 'pending' ? 'warning' : ($booking->status === 'cancelled' ? 'danger' : 'info')) }}">
                        ...
                        <td data-col="actions" onclick="event.stopPropagation()">
                            ...
                            <form action="{{ route('bookings.destroy', $booking) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete?')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="13" class="text-center text-muted py-4">No bookings found</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
@php
    $defaultPrefs = [
        'columns' => ['no' => true, 'wip' => true, 'plate' => true, 'customer' => true, 'phone' => false, 'date' => true, 'time' => false, 'service' => true, 'foreman' => false, 'sa' => false, 'notes' => false, 'status' => true, 'actions' => true],
        'order' => ['no', 'wip', 'plate', 'customer', 'phone', 'date', 'time', 'service', 'foreman', 'sa', 'notes', 'status', 'actions'],
        'widths' => [],
        'sort' => 'booking_date',
        'dir' => 'desc'
    ];
    $storedPrefs = auth()->user()?->booking_preferences ?? [];
    ...
    // Apply saved sort if no URL sort params
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const columnLabels = {
        'no': '#', 'wip': 'WIP', 'plate': 'Plate No', 'customer': 'Customer', 'phone': 'Phone',
        'date': 'Booking Date', 'time': 'Time', 'service': 'Service Type', 'foreman': 'Foreman',
        'sa': 'Service Advisor', 'notes': 'Notes', 'status': 'Status', 'actions': 'Actions'
    };
    ...
    // Auto-apply saved sort on first load if no URL params
    @if(!request()->has('sort'))
    if (userSort && userSort !== 'booking_date') {
        // Redirect to apply saved sort
        const url = new URL(window.location);
        url.searchParams.set('sort', userSort);
        url.searchParams.set('dir', userDir);
